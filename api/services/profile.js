...
async function getMultiple(page = 1){
  ...
  const rows = await db.query(
    `SELECT Email, Username, Password, B_Date, Name, Profile_Pic, User_Email
...
async function update(Email, profile){
  const result = await db.query(
    `UPDATE profile 
    SET Email=${profile.Email}, Username=${profile.Username}, Password=${profile.Password}, B_Date=${profile.B_Date}, Name=${profile.Name}, Profile_Pic=${profile.Profile_Pic}, User_Email=${profile.User_Email}
    WHERE Email=${Email}` 
  );
